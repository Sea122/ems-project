...
# --- DATA ---
employees_data = [
    {"id": "RD6723849001", "password": "ABCDE12345", "name": "Rangsan Somkane", "role": "Head of Department", "dept": "Management", "status": "Active", "is_first_login": False, "avatar": "https://ui-avatars.com/api/?name=Rangsan+Somkane&background=0D8ABC&color=fff"}
]
...
# --- NEW: บันทึกความเคลื่อนไหวล่าสุด ---
activities_data = [
...
def add_employee():
    ...
    log_activity(f"New staff added: {name}") # Log
...
def edit_employee():
    ...
    for emp in employees_data:
        if emp['id'] == emp_id:
            ...
            log_activity(f"Updated info for: {request.form['name']}") # Log
            break
    return redirect(url_for('hr'))
...
def delete_employee(emp_id):
    ...
    log_activity(f"Removed staff: {name_to_delete}") # Log
    return redirect(url_for('hr'))
...
def add_task():
    ...
    log_activity(f"New task assigned: {title}") # Log
...
def edit_task():
    ...
    for task in projects_data:
        if task['id'] == task_id:
            ...
            log_activity(f"Task updated: {request.form['title']}") # Log
            break
    return redirect(url_for('tasks'))
...
def delete_task(task_id):
    ...
    log_activity("A task was deleted.") # Log
    return redirect(url_for('tasks'))
...
def add_announcement():
    ...
    log_activity(f"New announcement: {title}") # Log
    return redirect(url_for('dashboard'))
...
def edit_announcement():
    ...
    for ann in announcements_data:
        if ann['id'] == ann_id:
            ...
            log_activity(f"Edited announcement: {request.form['title']}") # Log
            break
    return redirect(url_for('dashboard'))
...
def delete_announcement(ann_id):
    ...
    log_activity("Announcement removed.") # Log
    return redirect(url_for('dashboard'))
